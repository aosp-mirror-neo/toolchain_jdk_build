...
# Usage:
#   build-openjdk17-darwin.sh [-q] [-d <dist_dir>] build_dir
...
[[ -n "${quiet:-}" ]] || set -x
(cd "$build_dir" &&
   PATH="$autoconf_dir/bin":$PATH bash +x "$top/toolchain/jdk/jdk17/configure" \
     ${quiet:+--quiet} \
     --with-boot-jdk="$top/prebuilts/jdk/jdk17/darwin-x86" \
     --disable-full-docs \
